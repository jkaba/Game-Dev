...
// verifyGridStatus
// Verifies status of the custom grid
...
bool Sudoku::solveGrid(){
    ...
    // If there is no unassigned location, finish
    if(!FindUnassignedLocation(this -> grid, row, cell))
        return true;
...
void Sudoku::genPuzzle(){
    for(int i = 0; i < 81; i++){
        ...
        // If more than 1 solution, replace removed cell back
        int check = 0;
        countSoln(check);
        if(check != 1){
            this -> grid[x][y] = temp;
...
// Calculate branch difficulty score
...
// calculateDifficulty
// Calculate difficulty level of current grid
...
int main(int argc, char const *argv[]){
    ...
    // Initializing seed for RNG
    srand(time(NULL));
    
    // Creating a sudoku instance
    Sudoku *puzzle = new Sudoku();
    
    // Create a seed for puzzle generation
    puzzle -> createSeed();
